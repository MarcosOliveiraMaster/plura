...
import type { Page, Company, Collaborator } from '../App'
...
const CATEGORY_GRADIENT: Record<string, string> = {
  ...
  'espaco-eventos': 'linear-gradient(135deg,#3b82f6,#1d4ed8)',
  'passeio-turistico': 'linear-gradient(135deg,#06b6d4,#0891b2)',
...
const A11Y_LABELS: Record<string, string> = {
  'rampa': 'Rampa de acesso', 'elevador': 'Elevador acessível', 'banheiro': 'Banheiro adaptado',
  'vaga-pcd': 'Vaga PCD', 'piso-tatil': 'Piso tátil', 'libras': 'Atendimento em Libras',
  'braile': 'Materiais em Braille', 'cadeira-rodas': 'Cadeira de rodas',
  'audiodescricao': 'Audiodescrição', 'entrada-acessivel': 'Entrada acessível',
}
...
const SectionHeader = ({ label, title }: { label: string; title: string }) => (
  <div style={{ marginBottom: '1.25rem' }}>
    <p style={{
      ...
      textTransform: 'uppercase', marginBottom: '0.25rem', transition: 'color 350ms ease',
    }}>{label}</p>
    <h3 style={{
...
const CollaboratorModal = ({
  company, onClose, onAdd, onRemove, isOwner,
}: {
  company: Company
  onClose: () => void
  ...
  const handleAdd = () => {
    ...
    const alreadyExists = company.collaborators.some(c => c.email === email.trim())
    if (alreadyExists) { setError('Este e-mail já foi convidado'); return }
    setError('')
    setLoading(true)
    setTimeout(() => {
      onAdd(email.trim())
      ...
      <div className="r-modal" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div>
            ...
            <h3 style={{ fontSize: '1.125rem', fontWeight: 800, color: 'var(--c-text-1)', letterSpacing: '-0.03em' }}>
              Gerenciar equipe
            </h3>
          </div>
          <button
            onClick={onClose}
            style={{
              background: 'var(--c-glass-bg-sm)', border: 'var(--c-border-sm)',
        ...
        {isOwner && (
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{
              ...
              color: 'var(--c-text-2)', marginBottom: '0.5rem',
            }}>Convidar por e-mail</label>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input
                ...
                style={{
                  ...
                  border: error ? '1px solid #ef4444' : '1px solid var(--c-input-border)',
                  borderRadius: '0.75rem', color: 'var(--c-input-text)',
                  fontSize: '0.875rem', fontFamily: 'inherit', outline: 'none',
                  transition: 'border 200ms ease, background 350ms ease',
                }}
            ...
            {error && (
              <p style={{ marginTop: '0.375rem', fontSize: '0.8125rem', color: '#ef4444' }}>{error}</p>
            )}
            {sent && (
              <p style={{ marginTop: '0.375rem', fontSize: '0.8125rem', color: '#4ade80' }}>
        ...
        <div>
          <p style={{
            fontSize: '0.75rem', fontWeight: 600, color: 'var(--c-text-3)',
          ...
          {company.collaborators.length === 0 ? (
            <div style={{
              ...
              color: 'var(--c-text-4)', fontSize: '0.875rem',
            }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
              {company.collaborators.map((collab: Collaborator) => (
                <div key={collab.id} style={{
                  ...
                  <div style={{
                    ...
                    background: 'linear-gradient(135deg,rgba(26,122,255,0.3),rgba(0,98,230,0.2))',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{
                      fontSize: '0.875rem', fontWeight: 600, color: 'var(--c-text-1)',
                      marginBottom: '0.125rem', transition: 'color 350ms ease',
                  ...
                  {isOwner && (
                    <button
                      ...
                      style={{
                        ...
                        color: 'var(--c-text-4)', padding: '0.25rem',
                      ...
                      onMouseLeave={e => {
                        (e.currentTarget as HTMLButtonElement).style.color = 'var(--c-text-4)'
...
interface CompanyProfilePageProps {
  ...
  company:              Company
  isOwner:              boolean
  onAddCollaborator:    (companyId: string, email: string) => void
  onRemoveCollaborator: (companyId: string, collaboratorId: string) => void
  onEditCompany:        (companyId: string) => void
}

export default function CompanyProfilePage({
  onNavigate, company, isOwner,
  onAddCollaborator, onRemoveCollaborator, onEditCompany,
  ...
  const coverSrc  = company.photos[0] ?? null
    ...
    <>
      ...
      {showCollabModal && (
        <CollaboratorModal
          ...
          onAdd={email => onAddCollaborator(company.id, email)}
          onRemove={collaboratorId => onRemoveCollaborator(company.id, collaboratorId)}
        />
      )}
      ...
      <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        {/* ── Cover ──────────────────────────────────────────────────────── */}
        <div className="r-cover" style={{
          height: '280px', width: '100%', position: 'relative',
          background: coverSrc ? `url(${coverSrc}) center/cover no-repeat` : gradient,
        }}>
          ...
          {/* Top nav */}
          <div className="r-cover-back-group" style={{
            position: 'absolute', top: '1.5rem', left: '1.5rem', right: '5rem',
            display: 'flex', alignItems: 'center', gap: '0.75rem',
          }}>
            <button
              className="r-cover-back"
          ...
          {/* Owner actions */}
          {isOwner && (
            <div className="r-cover-edit-group" style={{
              position: 'absolute', top: '1.5rem', right: '1.5rem',
              display: 'flex', gap: '0.5rem',
            }}>
              <button
                className="r-cover-action-btn"
                onClick={() => setShowCollabModal(true)}
                style={{
                  display: 'flex', alignItems: 'center', gap: '0.375rem',
                  background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(12px)',
                  border: '1px solid rgba(255,255,255,0.15)', borderRadius: '9999px',
                  padding: '0.5rem 1rem', color: '#fff', fontSize: '0.8125rem',
                  fontFamily: 'inherit', cursor: 'pointer', transition: 'background 150ms ease',
                }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(0,0,0,0.65)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'rgba(0,0,0,0.45)')}
              >
              ...
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: '0.375rem',
                  background: 'rgba(26,122,255,0.75)', backdropFilter: 'blur(12px)',
                  border: '1px solid rgba(26,122,255,0.5)', borderRadius: '9999px',
                  padding: '0.5rem 1rem', color: '#fff', fontSize: '0.8125rem',
                  fontFamily: 'inherit', cursor: 'pointer', transition: 'background 150ms ease',
                }}
          ...
          {/* Avatar */}
          <div className="r-cover-avatar" style={{
            position: 'absolute', bottom: '-36px', left: '2rem',
            width: '72px', height: '72px', borderRadius: '1rem',
            background: gradient, border: '3px solid var(--c-bg)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{ padding: '0 1.5rem', maxWidth: '960px', margin: '0 auto' }}>
          <div className="r-profile-head r-profile-head-row" style={{
            paddingTop: '3rem', paddingBottom: '1.5rem',
            borderBottom: '1px solid var(--c-divider)',
            display: 'flex', flexWrap: 'wrap',
            alignItems: 'flex-end', justifyContent: 'space-between', gap: '1rem',
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.375rem' }}>
                ...
                <Badge variant="blue" size="sm">{company.categoryLabel}</Badge>
              </div>
              {company.address && (
                <p style={{
                  display: 'flex', alignItems: 'center', gap: '0.375rem',
                  fontSize: '0.9375rem', color: 'var(--c-text-3)',
                  marginBottom: '0.75rem', transition: 'color 350ms ease',
                }}>
              ...
              <div className="r-social-row" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                {company.instagram && (
                  <span style={{
                    ...
                    padding: '0.3rem 0.75rem', borderRadius: '9999px', fontSize: '0.8125rem',
                    background: 'var(--c-glass-bg-sm)', border: '1px solid var(--c-input-border)',
                    color: 'var(--c-text-2)', transition: 'background 350ms ease, color 350ms ease',
                  }}>
          ...
          {/* ── Content sections ─────────────────────────────────────────── */}
          <div className="r-profile-content-grid" style={{
            display: 'grid',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <GlassCard>
                ...
                <p style={{
                  ...
                  lineHeight: 1.7, transition: 'color 350ms ease',
                  ...
                  <div style={{
                    ...
                    {company.photos.map((url, i) => (
                      <div
                        ...
                        style={{
                          aspectRatio: '4/3', borderRadius: '0.75rem',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <GlassCard>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '0.9375rem', color: 'var(--c-text-1)', transition: 'color 350ms ease' }}>{company.categoryLabel}</p>
                  </div>
                  <div>
                    ...
                    <p style={{ fontSize: '0.9375rem', color: 'var(--c-text-1)', transition: 'color 350ms ease' }}>
                      {new Date(company.createdAt).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}
                    </p>
                  </div>
                </div>
              </GlassCard>
              ...
              <GlassCard>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                  ...
                  {isOwner && (
                    <Button variant="secondary" size="sm" icon={<UserPlusIcon />} iconPosition="left"
                      onClick={() => setShowCollabModal(true)}>
                      Adicionar
                    </Button>
                  )}
                    ...
                    {company.collaborators.map((collab: Collaborator) => (
                      <div key={collab.id} style={{
                        ...
                        <div style={{
                          ...
                          border: '1px solid rgba(26,122,255,0.35)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          color: '#6aadff', fontSize: '0.75rem',
                        }}>
                          {(collab.name ?? collab.email)[0].toUpperCase()}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{
                            fontSize: '0.8125rem', fontWeight: 600, color: 'var(--c-text-1)',
                            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                            transition: 'color 350ms ease',
                          }}>
                            {collab.name ?? collab.email}
                        ...
                        {isOwner && (
                          <button
                            onClick={() => onRemoveCollaborator(company.id, collab.id)}
                            style={{
                              background: 'none', border: 'none', cursor: 'pointer',
                              color: 'var(--c-text-4)', padding: '0.2rem',
                              borderRadius: '0.375rem', display: 'flex',
                              transition: 'color 150ms ease, background 150ms ease',
                            }}
                            onMouseEnter={e => {
      ...
      <style>{`
        @media (max-width: 640px) {
          .r-social-row span { font-size: 0.75rem !important; }
        }
      `}</style>
    </>
  )

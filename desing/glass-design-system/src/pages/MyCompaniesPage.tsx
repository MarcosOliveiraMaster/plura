...
const A11Y_LABELS: Record<string, string> = {
  'rampa': 'Rampa', 'elevador': 'Elevador', 'banheiro': 'Banheiro adapt.',
  'vaga-pcd': 'Vaga PCD', 'piso-tatil': 'Piso tátil', 'libras': 'Libras',
  'braile': 'Braille', 'cadeira-rodas': 'Cadeira de rodas',
...
const CATEGORY_GRADIENT: Record<string, string> = {
  ...
  'espaco-eventos': 'linear-gradient(135deg,#3b82f6cc,#1d4ed866)',
  'passeio-turistico': 'linear-gradient(135deg,#06b6d4cc,#0891b266)',
...
const CompanyCard = ({
  ...
  const visibleA11y = company.accessibility.slice(0, 3)
  const extraA11y   = company.accessibility.length - 3

  return (
    <div style={{
      background: 'var(--c-glass-bg)', backdropFilter: 'blur(16px)',
      WebkitBackdropFilter: 'blur(16px)', border: 'var(--c-border)',
      borderRadius: '1.25rem', boxShadow: 'var(--c-shadow-md)',
      overflow: 'hidden',
      transition: 'transform 200ms ease, box-shadow 200ms ease, background 350ms ease',
    }}
      onMouseEnter={e => {
        (e.currentTarget as HTMLDivElement).style.transform  = 'translateY(-4px)'
        ;(e.currentTarget as HTMLDivElement).style.boxShadow = '0 20px 48px rgba(0,0,0,0.35),inset 0 1px 0 rgba(255,255,255,0.14)'
      }}
      onMouseLeave={e => {
    ...
    >
      ...
      <div style={{
        height: '110px',
        background: company.photos[0] ? `url(${company.photos[0]}) center/cover no-repeat` : gradient,
        position: 'relative',
      }}>
        <div style={{ position: 'absolute', top: '0.75rem', right: '0.75rem' }}>
          <Badge variant="white" size="sm">{company.categoryLabel}</Badge>
        </div>
        {isMock && (
          <div style={{ position: 'absolute', top: '0.75rem', left: '0.75rem' }}>
            <Badge variant="blue" size="sm">Exemplo</Badge>
          </div>
        )}
        <div style={{
          position: 'absolute', bottom: '-20px', left: '1.25rem',
          width: '44px', height: '44px', borderRadius: '0.75rem',
          background: gradient, border: '2.5px solid var(--c-bg)',
      ...
      <div style={{ padding: '1.625rem 1.25rem 1.25rem' }}>
        <h3 style={{
          ...
          color: 'var(--c-text-1)', marginBottom: '0.2rem', transition: 'color 350ms ease',
        }}>{company.name}</h3>
        <p style={{
          fontSize: '0.8125rem', color: 'var(--c-text-3)',
          marginBottom: '0.625rem', transition: 'color 350ms ease',
        }}>{company.address || 'Endereço não informado'}</p>
        <p className="company-desc" style={{
          fontSize: '0.875rem', color: 'var(--c-text-2)',
          lineHeight: 1.5, marginBottom: '0.875rem', transition: 'color 350ms ease',
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <Button variant="primary" size="sm" icon={<EyeIcon />} iconPosition="left"
            style={{ flex: 1, justifyContent: 'center' }} onClick={onView}>
            Ver empresa
          </Button>
...
export default function MyCompaniesPage({
  onNavigate, companies, onEditCompany, onViewCompany,
    ...
    <>
      ...
      <div className="r-layout" style={{ minHeight: '100vh', position: 'relative', zIndex: 1, padding: '5.5rem 1.5rem 3rem' }}>
        <div className="r-container" style={{ maxWidth: '960px', margin: '0 auto' }}>
          ...
          {/* Header */}
          <div className="r-section-header" style={{
            display: 'flex', alignItems: 'center',
            justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem',
          }}>
            <div>
              <h1 style={{
                fontSize: '1.625rem', fontWeight: 800, letterSpacing: '-0.035em',
                color: 'var(--c-text-1)', marginBottom: '0.25rem', transition: 'color 350ms ease',
              }}>Minhas Empresas</h1>
          ...
          {/* Grid */}
          <div className="r-companies-grid" style={{
            ...
            {companies.map(company => (
              <CompanyCard
                key={company.id}
                company={company}
                isMock={company.id === 'mock-vista-verde'}
          ...
          <footer style={{
            marginTop: '3rem', textAlign: 'center', fontFamily: 'var(--font-mono)',
            fontSize: '0.6875rem', color: 'var(--c-text-4)',
            display: 'flex', gap: '1.25rem', alignItems: 'center', justifyContent: 'center',
            transition: 'color 350ms ease',
          }}>
